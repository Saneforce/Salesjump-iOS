...
//                    }
    ...
    func schemeReEditAction () {
        ...
        for prod in selectedProducts {
            ...
            let taxPer = product.taxper
            
            let unitCount = product.unitCount
            let rate = product.rate
            let sQty = Int(product.sampleQty) ?? 0
            
            let id = VisitData.shared.Dist.id
            
            if product.isMultiSchemeActive == true && product.stockistCode.contains(id) {
                let totalQty = unitCount * sQty
                let scheme = self.nextLessThanValue(in: product.multiScheme, comparedTo: totalQty,rate: rate)
                if scheme != nil {
                    discountPer = scheme!.disCountPer
                    product.disCountPer = scheme!.disCountPer
                }
            }else {
            ...
            if product.isSchemeActive == true && product.stockistCode.contains(id) {
                let totalQty = unitCount * sQty
                
                if product.package == "N" {
                    if product.isMultiSchemeActive == true {
                        ...
                        if scheme != nil {
                            ...
                            if scheme!.schemeType == "Q" {
                                if scheme!.discountType == "%" {
                                    ...
                                    product.freeCount = Int(value * Double(scheme!.offerAvailableCount))
                                }else {
                                    let schQty = scheme!.scheme
                                    let value = Double(totalQty) /  Double(schQty)
                                    product.freeCount = Int(value * Double(scheme!.offerAvailableCount))
                                    if Double(totalQty) >= Double(schQty){
                                        ...
                                        product.disCountPer = discountPerRound
                                        product.disCountAmount = disCountValuePer
    ...
    //                                cell.product.freeCount = Int(value * Double(scheme!.offerAvailableCount))
                                    ...
                                    if Double(total) >= Double(schQty){
                                        product.freeCount = Int(Double(value) * Double(scheme!.offerAvailableCount))
                        ...
                        if product.schemeType == "Q" {
                            if product.discountType == "%" {
                                let schQty = product.scheme
                                let value = Double(totalQty) /  Double(schQty)
                                if Double(totalQty) >= Double(schQty){
                                    product.freeCount = Int(value * Double(product.offerAvailableCount))
                                }else {
                                    product.freeCount = 0
                                }
                            }else {
                                ...
                                if Double(totalQty) >= Double(schQty){
                                    ...
                                    let amt = Int(Double(totalQty) / Double(product.scheme))
                                    let disCountValuePer = Double(amt)  * product.disCountValue
                        ...
                        }else if product.schemeType == "V" {
                            if product.discountType == "%" {
                                let schQty = product.scheme
                                let total = Double(sQty) * rate * Double(unitCount)
                                let value = Int(Double(total) /  Double(schQty))
                                if Double(total) >= Double(schQty){
                                    product.freeCount = Int(Double(value) * Double(product.offerAvailableCount))
                                }else{
                                    product.freeCount = 0
                                    product.disCountPer = 0
                                    product.disCountAmount = 0
                                ...
                                if Double(total) >= Double(schQty){
                                    ...
                                    let amt = Int(Double(total) / Double(product.scheme))
                                    let disCountValuePer = Double(amt)  * product.disCountValue
                    ...
                    if product.isMultiSchemeActive == true {
                        ...
                        if scheme != nil {
                            if scheme?.schemeType == "Q" {
                                ...
                                product.freeCount = schemeQty * scheme!.offerAvailableCount //  Int(value * Double(scheme!.offerAvailableCount))
                            }else {
                                let total = Double(sQty) * rate * Double(unitCount)
                                
                                print(total)
                                if Int(total / Double(product.scheme)) >= 1 {
                                    print(Int(total / Double(product.scheme)))
                                    product.freeCount = Int(total / Double(product.scheme)) * product.offerAvailableCount
                                    
                                    let amt = Int(total / Double(product.scheme))
                                    let disCountValuePer = Double(amt)  * product.disCountValue
                        ...
                        if product.schemeType == "Q" {
                            ...
                            if product.discountType == "%" {
                                let schemeQty = totalQty / product.scheme
                                if totalQty >= product.scheme{
                                    product.freeCount = schemeQty * product.offerAvailableCount
                                }else {
                                    product.freeCount = 0
                                }
                            }else {
                                ...
                                print(total)
                                if Int(Double(totalQty) / Double(product.scheme)) >= 1 {
                                    print(Int(total / Double(product.scheme)))
                                    product.freeCount = Int(Double(totalQty) / Double(product.scheme)) * product.offerAvailableCount
            ...
            if product.schemeType == "Q" {
                ...
                if product.discountType == "%" {
                  ...
                  //  txtDisAmt.text = "\(discountAmountRound)"
                 //   txtTaxAmt.text = "\(taxAmountRound)"
                    product.taxAmount = taxAmountRound
                    product.disCountAmount = discountAmountRound
                    product.totalCount =  totalAmountRound
                 //   lblRate.text = "\(product.rate) x ( \(unitCount) x \(sQty) )  =  \(totalAmountRound)"
                }else {
                    let discountAmount = product.disCountAmount // discountPer * Double(unitCount) * rate * Double(sQty) / 100
            ...
            if let index = self.allProducts.firstIndex(where: { (productInfo) -> Bool in
                return product.productId == productInfo.productId && product.retailerPrice == productInfo.retailerPrice
            }){
            ...
            for item in Editobjcalls {
                ...
                if let Additional_Prod_Dtls = item["Additional_Prod_Dtls"] as? String, let Additional_Prod_Code = item["Additional_Prod_Code"] as? String{
                    ...
                    for (separt_item, separte_Prod_Code_item) in zip(separates, separte_Prod_Code) {
                        ...
                        print(components) // "Fogg red 100 ml~375.44$9@0.5+0%10-40.5@@9?2-Piece^0*45!0_0/0..:%"
                        
                        let qtycon = components[3].components(separatedBy: "?")
                        sQty = qtycon[0]
                        let free = components[1].components(separatedBy: "%")
                        let freecountt = free[0].components(separatedBy: "+")
                        freecnt = freecountt[1]
                        let dispers = components[1].components(separatedBy: "%")
                        let disperv = dispers[1].components(separatedBy: "-")
                        discountPer = disperv[0]
                        discountValue = disperv[1]
                        let discntTy = components[3].components(separatedBy: "..:")
                        discountType = discntTy[1]
                    ...
                    if let baseUnitCodeStr = lstAllProducts[indexToDelete!]["Base_Unit_code"] as? String,
                       let baseUnitCodeInt = Int(baseUnitCodeStr) {
                        ...
                        updateQty(id: Prod_Id, sUom: Uomid2, sUomNm: selUOMNm, sUomConv: UomConQtydata2,sNetUnt: selNetWt, sQty: sQty,ProdItem: lProdItem, rateValue: rate, clQty: clQty,rateEdited: rateEdited,retailerPriceValue: retailerPrice,disPer: discountPer,disValue: discountValue,disType: discountType,freecnt: freecnt)
                }
            }
        }
    }
    
    func updateQty (id: String,sUom: String,sUomNm: String,sUomConv: String,sNetUnt: String,sQty: String,ProdItem:[String: Any],rateValue : String,clQty : String,rateEdited: String,retailerPriceValue : String,disPer : String,disValue : String,disType : String,freecnt : String) {
   ...
   //         self.allProducts.append(ProductList(product: product, productName: productName, productId: productId,cateId: cateId, rate: rate,rateEdited: "0",retailerPrice: retailorPrice,saleErpCode: saleErpCode,newWt: newWt, sampleQty: "",clQty: "",remarks: "",remarksId: "", selectedRemarks: [], disCountPer: disCountPer, disCountAmount: 0.0, freeCount: 0, unitId: unitId, unitName: unitName, unitCount: unitCount, taxper: tax, taxAmount: 0.0, totalCount: 0.0, isSchemeActive: isSchemeActive,scheme: scheme,offerAvailableCount: offerAvailableCount,offerUnitName: offerUnitName,offerProductCode: offerProductCode,offerProductName: offerProductName,package: package, isMultiSchemeActive: isMultiSchemeActive, multiScheme: multiScheme, competitorProduct: []))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == self.tbProductTableView {
            ...
            Cell.txtDisPer.addTarget(target: self, action: #selector(addDiscount(_:)))
            if UserSetup.shared.SchemeBased == 1 && UserSetup.shared.offerMode == 1{
                ...
                Cell.imgFreeEdit.isHidden = true
            }else if UserSetup.shared.SchemeBased == 0 && UserSetup.shared.offerMode == 1 {
                Cell.txtDisPer.borderStyle = .roundedRect
                Cell.txtFreeQty.borderStyle = .roundedRect
                Cell.txtDisPer.isUserInteractionEnabled = true
                Cell.txtFreeQty.isUserInteractionEnabled = true
                Cell.lblPer.text = products[indexPath.row].discountType == "Rs" ? UserSetup.shared.currency_symbol : products[indexPath.row].discountType
                Cell.lblPer.addTarget(target: self, action: #selector(selectDiscountMode))
                Cell.imgFreeEdit.addTarget(target: self, action: #selector(freeEditAction))
                
                let text = products[indexPath.row].discountType == "Rs" ? UserSetup.shared.currency_symbol : products[indexPath.row].discountType
                ...
                Cell.lblPer.attributedText = fullString
            }else {
                Cell.txtDisPer.borderStyle = .none
                Cell.txtFreeQty.borderStyle = .none
                Cell.txtDisPer.isUserInteractionEnabled = false
                Cell.txtFreeQty.isUserInteractionEnabled = false
                Cell.imgFreeEdit.isHidden = true
            }
            ...
            let distId = VisitData.shared.Dist.id
        ...
        //    Cell.txtDisPer.text = "\(products[indexPath.row].disCountPer)"
          //  Cell.txtDisAmt.text = "\(products[indexPath.row].disCountAmount)"
            return Cell
    ...
    func calculationForOrderCell(cell : SuperStockistOrderListTableViewCell) {
        ...
        if sQty == 0 {
            if UserSetup.shared.SchemeBased == 0 &&  UserSetup.shared.offerMode == 1{
                cell.product.disCountPer = 0
                cell.product.freeCount = 0
            }
        }
...
//                                cell.product.freeCount = Int(value * Double(scheme!.offerAvailableCount))
        ...
        if cell.product.isSchemeActive == true {
            ...
            if cell.product.discountType == "%" {
                ...
                let discountAmount = cell.product.disCountPer // discountPer * Double(unitCount) * rate * Double(sQty) / 100
    ...
    @objc private func freeQtySelection(_ sender : UITapGestureRecognizer) {
        ...
        if cell.product.sampleQty.isEmpty{
            Toast.show(message: "Please Select Qty")
            return
        }
    ...
    @objc private func addDiscount (_ sender : UITapGestureRecognizer) {
//        let cell:SuperStockistOrderListTableViewCell = GlobalFunc.getTableViewCell(view: sender) as! SuperStockistOrderListTableViewCell
//        let tbView:UITableView = GlobalFunc.getTableView(view: sender)
//        let indxPath: IndexPath = tbView.indexPath(for: cell)!
        ...
        editView.rate = cell.product.disCountPer
        editView.titleStr = "Edit Discount"
        editView.updateRate = { dis in
            ...
            cell.product.disCountPer = dis as! Double
    ...
    @objc private func selectDiscountMode(_ sender: UITapGestureRecognizer) {
        let cell: SuperStockistOrderListTableViewCell = GlobalFunc.getTableViewCell(view: sender.view!) as! SuperStockistOrderListTableViewCell
        let tbView:UITableView = GlobalFunc.getTableView(view: sender.view!)
        let indxPath: IndexPath = tbView.indexPath(for: cell)!
        
        
        let mode1 : AnyObject = ["mode" : "%","name" : "%"] as AnyObject
        let mode2 : AnyObject = ["mode" : "Rs","name" : UserSetup.shared.currency_symbol ] as AnyObject
        
        var modes = [AnyObject]()
        modes.append(mode1)
        modes.append(mode2)
        
        let unitVC = ItemViewController(items: modes, configure: { (Cell : SingleSelectionTableViewCell, mode) in
            Cell.textLabel?.text = mode["name"] as? String
        })
        unitVC.title = "Select the Mode"
        unitVC.didSelect = { selectedMode in
            
            let text = selectedMode["name"] as? String ?? ""
            let fullString = NSMutableAttributedString(string: text)
            let image1Attachment = NSTextAttachment()
            image1Attachment.image = UIImage(named: "downArrow")
            let image1String = NSAttributedString(attachment: image1Attachment)
            fullString.append(image1String)
            cell.lblPer.attributedText = fullString
            cell.product.discountType = selectedMode["mode"] as? String ?? ""
            let discount = cell.product.disCountPer
    ...
    @objc private func freeEditAction(_ sender : UITapGestureRecognizer) {
        let cell: SuperStockistOrderListTableViewCell = GlobalFunc.getTableViewCell(view: sender.view!) as! SuperStockistOrderListTableViewCell
        let tbView: UITableView = GlobalFunc.getTableView(view: sender.view!)
        let indxPath: IndexPath = tbView.indexPath(for: cell)!
        ...
        let freeEditView = FreeEditViewController<Any>()
        freeEditView.qty = cell.product.freeCount
        freeEditView.updateFreeQty = { Qty in
            
            cell.product.freeCount = Qty as! Int
...
//        competitorProduct.show()
    ...
    @objc private func schemeAction(_ sender : UITapGestureRecognizer){
        ...
        print(cell.product.disCountPer)
...
//        }
    ...
    func OrderSubmit(sLocation: String,sAddress: String){
        ...
        for product in selectedProducts {
            ...
            if UserSetup.shared.SchemeBased == 1 && UserSetup.shared.offerMode == 1 && product.stockistCode.contains(id) {
                scheme = product.scheme
                offerProductCode = product.offerProductCode
                offerProductName = product.offerProductName
                freePCount = product.offerProductCode
